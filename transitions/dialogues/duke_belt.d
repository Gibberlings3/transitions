...
APPEND BELT
	...
	IF ~~ THEN BELT_QUESTS_SKIP_THIS
		...
		IF ~Global("#L_SarvQuests","GLOBAL",3) !Dead("winski2")~ THEN DO ~SetGlobal("#L_SarvQuests","GLOBAL",4)~ + BELT_QUEST_4a
		IF ~Global("#L_SarvQuests","GLOBAL",2) !Dead("SEMAJ")~ THEN DO ~SetGlobal("#L_SarvQuests","GLOBAL",3)~ + BELT_QUEST_3a
	...
	IF ~~ THEN BELT_NOW_WHAT
		...
		+ ~Global("#L_AcceptedRoom","GLOBAL",1)~ + @2265 /* ~I think I'll go up stairs and sleep for a week.~ */ DO ~SetGlobal("#L_SarvQuests","GLOBAL",100) SetGlobal("EndOfBG1","GLOBAL",1) SetGlobal("#L_StartCaelarAttack","GLOBAL",1) SetGlobalTimer("#L_SoloExitTimer","GLOBAL",ONE_ROUND)~ EXIT
		+ ~Global("#L_AcceptedRoom","GLOBAL",0) Global("#L_ImoenInPalace","GLOBAL",1) OR(3) PartyHasItem("#LKey01") PartyHasItem("#LKey02") PartyHasItem("#LKey03")~ + @2268 /* ~I'm going to settle into my new home, but I'll be back in a couple weeks to visit Imoen.~ */ DO ~SetGlobal("#L_SarvQuests","GLOBAL",100) SetGlobal("EndOfBG1","GLOBAL",1) SetGlobal("#L_StartCaelarAttack","GLOBAL",2) SetGlobal("#L_SkipFirstHalfCA","GLOBAL",1) SetGlobalTimer("#L_SoloExitTimer","GLOBAL",ONE_ROUND)~ EXIT
		...
		+ ~Global("#L_AcceptedRoom","GLOBAL",0) Global("#L_ImoenInPalace","GLOBAL",0) OR(3) PartyHasItem("#LKey01") PartyHasItem("#LKey02") PartyHasItem("#LKey03")~ + @2266 /* ~I'm going to settle into my new house and sleep for a week.~ */ DO ~SetGlobal("#L_SarvQuests","GLOBAL",100) SetGlobal("EndOfBG1","GLOBAL",1) SetGlobal("#L_StartCaelarAttack","GLOBAL",3) SetGlobalTimer("#L_SoloExitTimer","GLOBAL",ONE_ROUND)~ EXIT
		+ ~Global("#L_AcceptedRoom","GLOBAL",0) Global("#L_ImoenInPalace","GLOBAL",0) !PartyHasItem("#LKey01") !PartyHasItem("#LKey02") !PartyHasItem("#LKey03")~ + @2267 /* ~I think I'll go to Elfsong and celebrate for a week.~ */ DO ~SetGlobal("#L_SarvQuests","GLOBAL",100) SetGlobal("EndOfBG1","GLOBAL",1) SetGlobal("#L_StartCaelarAttack","GLOBAL",3) SetGlobalTimer("#L_SoloExitTimer","GLOBAL",ONE_ROUND)~ EXIT
	...
	IF ~~ THEN BELT_QUEST_8a
		...
		+ ~GlobalGT("#L_SentToKorlasz","GLOBAL",0)~ + @2184 /* ~I'm ready to go, but how do I find this crypt?~ */+ RESTART_KORLASZ
	...
	IF ~~ THEN RESTART_KORLASZ
		SAY @2185 /* ~Hold still, I'll send you there.~ */
		IF ~~ THEN DO ~ClearAllActions() StartCutSceneMode() StartCutSceneEx("#L_Cut04",TRUE)~ EXIT
	END
